...
@inline function make_func(::Type{F}, additional_field::A, y::T, inputs, gen::Int) where {A <: AdditionalField, F <: DiffableFunction, T <: Union{Real, AbstractArray}}
    gf = GradField(
        inputs,
        out_to_tensor(y, gen),
        gen
...
@inline function make_func(::Type{F},  y::T, inputs, gen) where {F <: DiffableFunction, T <: Union{Real, AbstractArray}}
    gf = GradField(
        inputs,
        out_to_tensor(y, gen),
        gen
...
@inline function make_func(::Type{F},  y::T, inputs, gen, device_idx) where {T <: CuArray, F <: DiffableFunction}
    gf = GradField(
        inputs,
...
function call!(F::Type{<:BinaryOperator}, x1::T, x2::S; nograd=false) where {T <: Variable, S <: Variable}
    inputs = (x1, x2)
...
function call!(F::Type{<:BinaryOperator}, x1::T, x2::T; nograd=false) where {T <: CuTensor}
...
function call!(F::Type{<:UnaryOperator}, additional_field::A, x::T, nograd=false) where {A <: AdditionalField, T<:Variable}
    if x.req_broadcast
...
function call!(F::Type{<:BinaryOperator}, additional_field::A, x1::T, x2::T; nograd=false) where {A <: AdditionalField, T<:Variable}
    inputs = (x1, x2)
...
function call!(F::Type{<:BinaryOperator}, additional_field::A, x1::T, x2::S; nograd=false) where {A <: AdditionalField, T <: Variable, S <: Variable}
    inputs = (x1, x2)
    y = forward(F,  additional_field, x1.values, x2.values)
    (nograd) && (return y)
    gen = min(x1.generation, x2.generation)
...
function call!(F::Type{<:BinaryOperator}, additional_field::A, x1::T, x2::T; nograd=false) where {A <: AdditionalField, T <: CuTensor}
    device_idx = check_same_device(x1.device, x2.device)
    inputs = (x1, x2)
    y = forward(F,  additional_field, x1.values, x2.values)
    (nograd) && (return y)
...
@inline function set_grad!(x::T, gx::S; nograd::Bool) where {T <: Variable, S <: Union{Real, AbstractArray}}
    if nograd
        set_grad!(x, x.grad, gx)
    end        
end
...
function backward!(y::Scalar; retain_grad=false, create_graph=false)
    ...
    while !(isempty(que))
        ...
        calculate_grad!(f, seen_set, que, retain_grad=retain_grad; create_graph=create_graph)
    end
    return nothing
end

function calculate_grad!(f::BinaryOperator, seen_set::Set{DiffableFunction}, que::PriorityQueue{DiffableFunction, Int}; retain_grad=false, create_graph=false)
    gy = get_gy(f)
...
function calculate_grad!(f::UnaryOperator, seen_set::Set{DiffableFunction}, que::PriorityQueue{DiffableFunction, Int}; retain_grad=false, create_graph=false)
    gy = get_gy(f)
